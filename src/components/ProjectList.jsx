...
/**
 * The project index rows, with the cursor-following image preview.
 *
 * `entries` is a list of `[name, project]` pairs; `animationKey` swaps the whole
 * list out through AnimatePresence whenever the caller changes filter.
 */
const ProjectList = ({ entries, animationKey = "list" }) => {
  ...
  // a filter change can leave a stale index pointing past the new list
  useEffect(() => {
    setHovered(null);
  }, [animationKey]);

  // width follows the image's aspect at a fixed height, clamped so a very wide
  // or very tall shot cannot take over the screen
  const measure = (img) => {
