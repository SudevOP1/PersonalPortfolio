...
const Projects = () => {
  ...
  return (
    <MainLayout>
      <h1 className="text-2xl sm:text-3xl text-slate-100 font-semibold">My Projects</h1>
        ...
        {Object.entries(projects[activeProject]).map(([projectName, project], i) => (
          <motion.div
            ...
            onClick={() =>
              window.open(
                project.links.find(
                  (link) => (link.name === "Code" && link.link.length > 0) || (link.name === "Live" && link.link.length > 0),
                )?.link,
                "_blank",
              )
            }
          >
            ...
            <div className="rounded-lg border border-slate-700/80 overflow-hidden shadow-md">
              <img src={project.img ? project.img : InProgress} className="group-hover:scale-105 transition" />
            </div>
            {/* name & desc */}
            <div className="flex flex-col">
              <h1 className="text-xl sm:text-2xl text-slate-100 font-semibold">{projectName}</h1>
              <h1 className="text-md sm:text-lg text-slate-400 font-semibold leading-[1.2]">{project.desc}</h1>
            </div>
            {/* stacks */}
            <div className="flex flex-wrap gap-2">
              {project.stacks.map((stack, j) => (
                <span key={j} className={`px-3 rounded-full text-md ${getSkillColor(stack)} `}>
                  {stack}
                </span>
              ))}
            </div>
              ...
              {project.links.map(
                (link, j) =>
                  link.link.length > 0 && (
                    ...
                    >
                      {link.name === "Code" ? <Github className="w-4 h-4" /> : <ExternalLink className="w-4 h-4" />}
                      {"View " + link.name}
                    </motion.a>
                  ),
              )}
