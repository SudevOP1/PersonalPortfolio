        ...
        {Object.entries(experiences).map(([name, e], i) => (
          ...
          >
            ...
            <a
              ...
              rel="noopener noreferrer"
            >
              <img src={e.img} className="rounded-full w-20 h-20 sm:w-25 sm:h-25 bg-white" />
              <div className="flex flex-col">
                <h1 className="text-xl sm:text-2xl text-slate-100 font-semibold">{name}</h1>
                <h1 className="text-md sm:text-lg text-slate-300 font-semibold">{e.role}</h1>
                <h1 className="text-xs sm:text-sm text-sky-400 font-semibold">{e.period}</h1>
              </div>
            </a>
            ...
            <div className="flex flex-wrap mt-3 gap-2 pl-2">
              {e.stacks.map((stack, j) => (
                <span key={j} className={`px-4 py-1 rounded-full text-xs sm:text-sm shadow-sm ${stack.color}`}>
                  {stack.stack}
                </span>
              ))}
            </div>
            ...
            {["Personal", "Client"].map((projectType, i) => (
              <button
                ...
                className={`w-1/2 py-2 rounded-full transition text-md font-semibold cursor-pointer ${
                  activeProject === projectType ? "bg-slate-400 text-slate-900" : "text-slate-400 hover:bg-slate-500/40"
                }`}
              >
                {projectType}
              </button>
        ...
        <div className="flex flex-wrap gap-5 mt-5">
          {Object.entries(projects[activeProject])
            ...
            .map(([name, project], i) => (
              <motion.div
                onClick={() => {
                  const link = project.links.find((link) => {
                    (link.name === "Code" && link.link.length > 0) || (link.name === "Live" && link.link.length > 0);
                  })?.link;
              ...
              >
                <div
                  ...
                  style={{
                    backgroundImage: `url(${project.img ? project.img : InProgress})`,
                  }}
                />
                ...
                <div className="flex flex-col flex-1 gap-1 absolute bottom-2 left-2 z-10">
                  <h1 className="text-md text-slate-100 font-semibold">{name}</h1>
                  <p className="text-slate-300 font-normal text-xs">{project.desc}</p>
                    ...
                    {project.links.map(
                      (link, j) =>
                        link.link.length > 0 && (
                          ...
                          >
                            {link.name === "Code" ? <Github className="w-3 h-3" /> : <ExternalLink className="w-3 h-3" />}
                            {"View " + link.name}
                          </a>
                        ),
                    )}
